...
while flag == 1
    filename = [rootname int2str(M + 1) '.xlsx'];
    ...
    if exist (filename) == 2 
        ...
        for row = 1:T                
                ...
                runfiles(M).resus(row) = numbers(row + 2,6); % bay resuspension elevation (m)
                runfiles(M).overwashrate(row) = numbers(row + 2,7); % DW - rate of overwash accretion - controls the length of overwash spreading
                runfiles(M).overwashflux(row) = numbers(row + 2,8); % DW - volume of overwash deposited onto te backbarrier from storms
                runfiles(M).highwater(row) = numbers(row + 2,9); % DW - highwater line from the tidal range
                runfiles(M).bayerosion(row) = numbers(row + 2,10); % DW - rate of bay erosion
        end         
    else 
